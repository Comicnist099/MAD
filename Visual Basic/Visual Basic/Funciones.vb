...
Error1:
    ...
    Public Shared Sub Proceso()
        ...
        obj.GenerarArchivo()
    End Sub
    Public Shared Function LogInFillData(ByVal nombre As String) As Boolean
        Dim UsuarioData As LogInData = LogInData.Instance
        Dim Enlace As New EnlaceBD
        Dim id As Integer = Enlace.GetIdUsuario(nombre)
        Dim Usuario As DataRow = Enlace.GetUser(id).Rows.Item(0)
        Dim UsuarioDatos As DataRow = Enlace.GetUser2(id).Rows.Item(0)
        Dim UsuarioRol As DataRow = Enlace.GetUser3(id).Rows.Item(0)
        MessageBox.Show(UsuarioDatos("fk_Empleado").ToString() + ": " + UsuarioDatos("Nombre").ToString() + " " + UsuarioDatos("Apellido_M").ToString() + " " + UsuarioDatos("Apellido_P").ToString(), "Acceso concedido: ", MessageBoxButtons.OK, MessageBoxIcon.Information)
        If Enlace.esempleado(id) Then
            ' Dim idempleado As Integer = Enlace.getidempleado(id)
            ' Dim empleado As DataRow = Enlace.GetEmpleadobyuser(idempleado).Rows.Item(0)
            UsuarioData.setIdUsuario(UsuarioDatos("fk_Empleado").ToString)
            UsuarioData.setIdPuesto(UsuarioRol("fk_Empleado").ToString)
            UsuarioData.setPuesto(UsuarioRol("rol"))
        End If
        Return True
    End Function

    Public Shared Function LogInFillDataUSER(ByVal nombre As String) As Boolean
        Dim UsuarioData As LogInData = LogInData.Instance
        Dim Enlace As New EnlaceBD
        Dim id As Integer = Enlace.GetIdUsuarioUsuarios(nombre)
        Dim Usuario As DataRow = Enlace.GetUserUsuario(id).Rows.Item(0)
        If Enlace.esUsuario(id) Then
            ' Dim idempleado As Integer = Enlace.getidempleado(id)
            ' Dim empleado As DataRow = Enlace.GetEmpleadobyuser(idempleado).Rows.Item(0)
            UsuarioData.setIdUsuario(id)
    ...
    'Dim id As Integer = Enlace.GetIdUsuario(nombre)
    'Dim Usuario As DataRow = Enlace.GetUser(id).Rows.Item(0)
    'MessageBox.Show(Usuario("ID").ToString() + ": " + Usuario("Usuario").ToString() + " " + Usuario("apellido materno").ToString() + " " + Usuario("apellido paterno").ToString(), "Acceso concedido: ", MessageBoxButtons.OK, MessageBoxIcon.Information)
    '     If Enlace.EsEmpleado(id) Then
    '   Dim idempleado As Integer = Enlace.getidempleado(id)
    '   Dim empleado As DataRow = Enlace.GetEmpleadobyuser(idempleado).Rows.Item(0)
    '  UsuarioData.setIdUsuario(Usuario("ID").ToString)
    ' UsuarioData.setAccessLevel(empleado("Administrador").ToString)
    ' UsuarioData.setIdPuesto(empleado("ID").ToString)
    ' UsuarioData.setPuesto("empleado")

    ' Else
    '   Dim idcliente As Integer = Enlace.getidcliente(id)
    '   Dim cliente As DataRow = Enlace.GetClientebyuser(idcliente).Rows.Item(0)
    '  UsuarioData.setIdUsuario(Usuario("ID").ToString)
    '   UsuarioData.setAccessLevel(0)
    '   UsuarioData.setIdPuesto(Cliente("ID").ToString)
    '  UsuarioData.setPuesto("cliente")
    ' End If
    ...
    'For Each row As DataRow In Usuario.Rows
    'If Not String.IsNullOrEmpty(row("Nombre").ToString()) Then
    ...
    Public Shared Function CleanTextBoxs(ByRef Ventana As Form) As Boolean
        Dim texto As Object
        For Each texto In Ventana.Controls
            If TypeOf texto Is TextBox Then
                Dim txtTemp As TextBox = CType(texto, TextBox)
                txtTemp.Text = ""
            End If
        Next
        Return True
